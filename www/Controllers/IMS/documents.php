    ...
    if (empty($_FILES['file'])) {
        if (isset($_GET['name'])) {
            ...
            try {
                ...
                $conn = new DbConn();
                $conn->selectDb('ims');
                $sql = "INSERT INTO documents (ims_category_id, doc_name, doc_category, doc_description, doc_revision, doc_attachment, doc_language, archive) 
                                    VALUES (
                                        (SELECT id FROM ims_categories WHERE category_name = ?),
                                        ?, ?, ?, ?, ?, ?, ?
                                    );";
                $stmt = $conn->prepareStmt($sql);
    ...
    try {
        ...
        try {
            ...
            if (isset($error)) {
                ...
                $fileStatus = null;
                $baseDIR = __DIR__ . '/../../Uploads/Documents/IMS/';
                $ext = '.pdf';
